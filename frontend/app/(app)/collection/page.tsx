...
export default function CollectionPage() {
  ...
  const [sortField, setSortField] = useState<"element_code" | "element_name" | "status">("element_code");
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((dp) => {
                    const statusCfg = STATUS_CONFIG[dp.status];
                    const boundaryCfg = BOUNDARY_CONFIG[dp.boundary_status];

                    return (
                      <tr
                        key={dp.id}
                        onClick={() => router.push(`/collection/${dp.id}`)}
