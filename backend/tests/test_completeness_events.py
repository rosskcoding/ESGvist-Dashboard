...
async def _setup_org_admin(client) -> dict:
    await client.post(
        "/api/auth/register",
        json={"email": "admin+compl@org.com", "password": "password123", "full_name": "Completeness Admin"},
    )
...
async def _invite_and_accept(client, *, admin_headers: dict, org_id: int, email: str, role: str, full_name: str):
...
async def test_completeness_change_publishes_webhook_and_manager_notification(monkeypatch, client):
    ...
    std = await client.post("/api/standards", json={"code": "CMP", "name": "Completeness"}, headers=org["headers"])
    disc = await client.post(
        f"/api/standards/{std.json()['id']}/disclosures",
        json={"code": "CMP-1", "title": "Completeness", "requirement_type": "quantitative", "mandatory_level": "mandatory"},
        headers=org["headers"],
    )
    item = await client.post(
        f"/api/disclosures/{disc.json()['id']}/items",
        json={"name": "Metric", "item_type": "metric", "value_type": "number", "is_required": True},
        headers=org["headers"],
    )
    ...
    project = await client.post("/api/projects", json={"name": "Completeness Project"}, headers=org["headers"])
    await client.post(
    ...
    await client.post(
        "/api/webhooks",
        json={"url": "https://example.com/hooks/completeness", "events": ["completeness.updated"]},
        headers=org["headers"],
    )
    await client.post(
    ...
    manager_notifications = await client.get("/api/notifications", headers=manager["headers"])
    assert any(item["type"] == "completeness_recalculated" for item in manager_notifications.json()["items"])
    ...
    manager_notifications = await client.get("/api/notifications", headers=manager["headers"])
    assert any(item["type"] == "completeness_100_percent" for item in manager_notifications.json()["items"])
