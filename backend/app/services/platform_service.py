...
class PlatformService:
    ...
    def _require_support_tenant_match(self, ctx: RequestContext, tenant_id: int) -> None:
        if (
            ctx.support_mode
            and ctx.organization_id is not None
            and ctx.organization_id != tenant_id
        ):
            raise AppError(
                "SUPPORT_SESSION_ORG_MISMATCH",
                409,
                "Support session tenant does not match requested tenant",
            )
    ...
    async def list_tenants(self, ctx: RequestContext, *, page: int, page_size: int):
        ...
        tenants = await self.repo.list_tenants(offset=(page - 1) * page_size, limit=page_size)
        items = []
        for org in tenants:
    ...
    async def create_tenant(
        self, ctx: RequestContext, *, name: str, country: str | None, industry: str | None
    ):
        self._require_platform(ctx)
    ...
    async def get_tenant(self, ctx: RequestContext, tenant_id: int):
        self._require_platform(ctx)
        self._require_support_tenant_match(ctx, tenant_id)
    ...
    async def set_tenant_status(self, ctx: RequestContext, tenant_id: int, new_status: str):
        ...
        await self._audit(
            ...
            entity_id=org.id,
            organization_id=org.id,
            action=f"platform_tenant_{new_status}",
        )
        return org
    ...
    async def assign_tenant_admin(self, ctx: RequestContext, tenant_id: int, user_id: int):
        ...
        if existing:
            raise AppError(
                "ROLE_BINDING_EXISTS", 409, "User already has a role in this organization"
            )
    ...
    async def get_self_registration(self, ctx: RequestContext):
        self._require_platform(ctx)
        ps = await self.repo.get_platform_settings()
        return ps.allow_self_registration

    async def set_self_registration(self, ctx: RequestContext, enabled: bool):
        self._require_platform(ctx)
        ps = await self.repo.update_platform_settings(allow_self_registration=enabled)
        await self._audit(
            ctx,
    ...
    async def end_support_session(self, ctx: RequestContext, session_id: int):
        ...
        await self._audit(
            ...
            entity_type="SupportSession",
            entity_id=ss.id,
            organization_id=ss.tenant_id,
            action="support_session_ended",
        )
        return ss

    async def get_current_support_session(self, ctx: RequestContext, session_id: int | None):
        self._require_platform(ctx)
        ...
        org = await self.repo.get_tenant(ss.tenant_id)
        if not org:
            return None
